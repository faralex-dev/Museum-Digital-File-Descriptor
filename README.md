...
проверяет их сохранность по разделу XXXIII «Хранение цифровых музейных
предметов» [Единых правил](https://base.garant.ru/74868877/) (приказ
Минкультуры России от 23.07.2020 № 827).

Для каждого предмета создаются:

- **файл метаданных** — XML в UTF-8: учётный номер, формат, размер, даты,
  контрольные суммы и технические характеристики
...
3. Заполните сведения о предмете. Учётный номер, если его не ввести, берётся из
   имени папки до первого «_»: `ГМИГ КП ЭФ-55_Петров А.А._Соловки` →
   `ГМИГ КП ЭФ-55`. Разделитель можно сменить на вкладке «Настройки»
   (в командной строке — `--separator`).
4. Нажмите «Создать описание».

Описания версии 1.x обновляются автоматически: программа сверяет файлы с их
контрольными суммами и переносит старые файлы описания в подпапку
`_описание 1.x` (подробнее — в [docs/format-2.0.md](docs/format-2.0.md)).
